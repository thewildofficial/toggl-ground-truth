...
class Store:
    ...
    def save_day(self, date: str, scores: Dict[str, int], maintenance: int, total: int, streaks: Dict[str, int]):
        conn = self._conn()
        conn.execute("""
            INSERT INTO daily_scores (date, scores, maintenance_seconds, total_tracked_seconds, streaks)
            VALUES (?, ?, ?, ?, ?)
        ...
        """, (date, json.dumps(scores), maintenance, total, json.dumps(streaks)))
        conn.commit()
        conn.close()

    def get_day(self, date: str) -> Optional[Dict]:
        conn = self._conn()
        row = conn.execute("SELECT * FROM daily_scores WHERE date = ?", (date,)).fetchone()
        conn.close()
        if not row:
            return None
        return {
            ...
            "maintenance": row[4],
            "total": row[5],
            "streaks": json.loads(row[6]),
        }

    def get_range(self, start: str, end: str) -> List[Dict]:
        conn = self._conn()
        rows = conn.execute(
            "SELECT * FROM daily_scores WHERE date >= ? AND date <= ? ORDER BY date",
            (start, end)
        ).fetchall()
        conn.close()
        return [
    ...
    def get_all_days(self) -> List[Dict]:
        conn = self._conn()
        rows = conn.execute("SELECT * FROM daily_scores ORDER BY date").fetchall()
        conn.close()
        return [
            {
                "date": r[0],
    ...
    def save_entries(self, entries: List[Dict]):
        ...
        for e in entries:
            conn.execute("""
                INSERT OR IGNORE INTO entries (toggl_id, date, description, duration, categories, is_maintenance, raw_json)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                e.get("id"),
                e.get("date"),
